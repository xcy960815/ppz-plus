 ...
 */
export class PgPostgreSqlMetadataProvider implements PostgreSqlMetadataProvider {
  /**
   ...
   * @returns {readonly} 归一化后的名称元数据。
   */
  private normalizeNameRows(
    rows: readonly Record<string, unknown>[],
  ): readonly { readonly name: string }[] {
    return rows
      .map((row) => (typeof row.name === "string" ? { name: row.name } : undefined))
      .filter((entry): entry is { readonly name: string } => entry !== undefined);
  }
}
