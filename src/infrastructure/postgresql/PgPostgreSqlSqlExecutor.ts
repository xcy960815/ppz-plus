...
export class PgPostgreSqlSqlExecutor implements PostgreSqlSqlExecutor {
  ...
  private normalizeResultSets(rawResult: unknown): readonly SqlExecutionResultSet[] {
    if (Array.isArray(rawResult)) {
      return rawResult.map((result) => this.normalizeResultSet(result as PgQueryResult));
    }
   ...
   */
  private readRows(rawResult: PgQueryResult): readonly Record<string, unknown>[] {
   ...
   */
  private normalizeMetadata(rawResult: PgQueryResult): readonly SqlExecutionResultMetadataEntry[] {
    return ["command", "rowCount", "oid"]
      ...
      .filter((entry): entry is SqlExecutionResultMetadataEntry => entry !== undefined);
  }
  ...
  private inferQuerySql(sql: string): boolean {
    const firstKeyword = sql
      ...
      .match(/^[a-z]+/i)?.[0]
      .toUpperCase();
    return ["SELECT", "SHOW", "EXPLAIN", "WITH", "TABLE", "VALUES"].includes(firstKeyword ?? "");
  }
}
