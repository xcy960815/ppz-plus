...
export class PgPostgreSqlExportProvider implements PostgreSqlExportProvider {
  ...
  public async exportTable(
    ...
    try {
      const blocks: string[] = [this.renderTableHeader(connection.name, target, kind)];
  ...
  public async exportSchema(
    ...
    try {
      ...
      const tableTargets = (await this.listSchemaTables(client, target)).map((tableName) => ({
        databaseName: target.databaseName,
        schemaName,
        tableName,
      }));
      const blocks: string[] = [this.renderSchemaHeader(connection.name, target, kind)];
      ...
      if (tableTargets.length === 0) {
        blocks.push(`-- ${target.databaseName}.${target.schemaName} 中未找到基础表。`);
      }
      ...
      if ((kind === "ddl" || kind === "both") && tableTargets.length > 0) {
        // 外键 ALTER TABLE 使用完整限定名，当前 search_path 不影响约束恢复。
        const foreignKeyBlock = await this.exportForeignKeys(client, tableTargets);
        if (foreignKeyBlock.length > 0) {
          blocks.push(foreignKeyBlock);
        }
      }
  ...
  public async exportDatabase(
    ...
    try {
      ...
      const blocks: string[] = [this.renderDatabaseHeader(connection.name, target, kind)];
      ...
      if (kind === "ddl" || kind === "both") {
        ...
        blocks.push(
          ...schemaNames.map((schemaName) => this.renderCreateSchemaStatement(schemaName)),
        );
      }
  ...
  private async openClient(
    ...
    const client = new postgreSql.Client(
      this.postgreSqlConnectionAdapter.resolveDriverOptions(connection, databaseName),
    );
   ...
   */
  private async listDatabaseSchemas(client: PgRuntimeClient): Promise<readonly string[]> {
    const result = await client.query(
      [
    ...
    return result.rows
      .map((row) => (typeof row.schemaName === "string" ? row.schemaName : undefined))
  ...
  private async listDatabaseTables(
    ...
    return result.rows
      ...
      .filter((table): table is SqlExportTableTarget => table !== undefined);
  }
  ...
  private async exportDdl(
    ...
    if (sequences.length > 0) {
      blocks.push(
        sequences.map((sequence) => this.renderCreateSequenceStatement(sequence)).join("\n"),
      );
    }
    ...
    if (sequences.length > 0) {
      blocks.push(
        sequences
          .map((sequence) => this.renderSequenceOwnershipStatement(target, sequence))
          .join("\n"),
      );
    }

    if (indexes.length > 0) {
      blocks.push(indexes.map((index) => this.renderIndexStatement(index)).join("\n"));
    }
  ...
  private async listColumns(
    ...
    return result.rows
      ...
      .filter((column): column is PostgreSqlColumnDefinition => column !== undefined);
  }
  ...
  private async listConstraints(
    ...
    return result.rows
      ...
      .filter(
        (constraint): constraint is PostgreSqlConstraintDefinition => constraint !== undefined,
      );
  }
  ...
  private async listIndexes(
    ...
    return result.rows
      ...
      .filter((index): index is PostgreSqlIndexDefinition => index !== undefined);
  }
  ...
  private async listOwnedSequences(
    ...
    return result.rows
      ...
      .filter((sequence): sequence is PostgreSqlSequenceDefinition => sequence !== undefined);
  }
  ...
  private renderColumnDefinition(column: PostgreSqlColumnDefinition): string {
    const fragments = [this.escapeIdentifier(column.columnName), column.dataType];
   ...
   */
  private renderTableConstraint(constraint: PostgreSqlConstraintDefinition): string {
   ...
   */
  private renderCreateSequenceStatement(sequence: PostgreSqlSequenceDefinition): string {
    return [
  ...
  private normalizeConstraintDefinition(
    ...
    if (typeof constraintName !== "string" || typeof constraintDefinition !== "string") {
      return undefined;
    }
  ...
  private normalizeIntegerText(value: unknown): string | undefined {
    ...
    if (typeof value === "string" && /^-?\d+$/.test(value)) {
      // 调用方只把该值写回整数 SQL 片段；正则保证字符串不携带表达式内容。
      return value;
    }
  ...
  private async exportDml(
    ...
    const result = await client.query(`SELECT * FROM ${this.escapeQualifiedTableName(target)}`);
  ...
  private formatSqlValue(value: unknown): string {
    ...
    if (ArrayBuffer.isView(value)) {
      return `decode('${Buffer.from(value.buffer, value.byteOffset, value.byteLength).toString(
        "hex",
      )}', 'hex')`;
    }
   ...
   */
  private escapeQualifiedSequenceName(sequence: PostgreSqlSequenceDefinition): string {
