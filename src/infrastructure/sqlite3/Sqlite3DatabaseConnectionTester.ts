...
export class Sqlite3DatabaseConnectionTester implements ConnectionTester {
  ...
  public async test(config: ConnectionConfig): Promise<void> {
    ...
    const databasePath = this.sqlite3ConnectionAdapter.resolveDatabasePath(config);
    ...
    const database = await this.openDatabase(databasePath, sqlite3.OPEN_READWRITE);
   ...
   */
  private async openDatabase(databasePath: string, mode: number): Promise<Sqlite3RuntimeDatabase> {
    const sqlite3 = await this.sqlite3RuntimeLoader.loadSqlite3Module();
