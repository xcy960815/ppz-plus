 ...
 */
export class Sqlite3MetadataProvider implements ApplicationSqlite3MetadataProvider {
  /**
   * 创建 SQLite3 元数据提供者。
   *
   ...
   */
  private normalizeTableRows(rows: Sqlite3QueryRows): readonly Sqlite3TableMetadata[] {
    return rows
      .map((row) => {
        const name = row.name;
        const type = row.type;
