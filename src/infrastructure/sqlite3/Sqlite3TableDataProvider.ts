 ...
 */
export class Sqlite3TableDataProvider implements ApplicationSqlite3TableDataProvider {
  /**
   * 创建 SQLite3 表数据提供者。
   *
  ...
  public async listRowPage(
    ...
    try {
      ...
      const rowCountQuery = this.createRowCountQuery(tableName, columns, options);
      ...
      return {
        ...
        sqlWithoutPagination: rowPageQuery.displaySqlWithoutPagination ?? rowPageQuery.displaySql,
  ...
  public async insertRow(
    ...
    try {
      ...
      const result = await this.run(database, insertQuery.sql, insertQuery.values);
  ...
  public async updateRow(
    ...
    try {
      const updateQuery = this.createUpdateRowQuery(tableName, identityValues, values);
      const result = await this.run(database, updateQuery.sql, updateQuery.values);
  ...
  public async deleteRow(
    ...
    try {
      ...
      const result = await this.run(database, deleteQuery.sql, deleteQuery.values);
  ...
  private async listColumnsWithDatabase(
    ...
    return rows
      .map((row): Sqlite3TableColumnMetadata | undefined => {
        ...
        return {
          name,
          dataType: typeof type === "string" && type.trim().length > 0 ? type : "TEXT",
      ...
      .filter((column): column is Sqlite3TableColumnMetadata => column !== undefined);
  }
  ...
  private createRowPageQuery(
    ...
    return {
      ...
      displaySqlWithoutPagination: this.createDisplaySql(sqlWithoutPagination, whereClause.values),
    };
  }
  ...
  private createWhereClause(
    ...
    if (keyword && columns.length > 0) {
      clauses.push(
        `(${columns
          .map((column) => `CAST(${this.quoteIdentifier(column.name)} AS TEXT) LIKE ?`)
          .join(" OR ")})`,
      );
  ...
  private createConditionClause(
    ...
    if (condition.operator === "in" || condition.operator === "not in") {
      ...
      return {
        sql: `${column} ${condition.operator.toUpperCase()} (${values.map(() => "?").join(", ")})`,
        values,
      };
    }
  ...
  private createInsertRowQuery(
    ...
    const columnNames = entries.map(([columnName]) => this.quoteIdentifier(columnName));
  ...
  private createUpdateRowQuery(
    ...
    const boundValues = [...updateEntries.map(([, value]) => value), ...whereClause.values];
   ...
   * @returns WHERE SQL 片段和绑定参数。
   */
  private createIdentityWhereClause(identityValues: Sqlite3TableRowIdentityValues): {
    readonly sql: string;
    readonly values: readonly unknown[];
  } {
   ...
   */
  private async openDatabase(connection: Sqlite3ConnectionConfig): Promise<Sqlite3RuntimeDatabase> {
    const sqlite3 = await this.sqlite3RuntimeLoader.loadSqlite3Module();
    const databasePath = this.sqlite3ConnectionAdapter.resolveDatabasePath(connection);
   ...
   */
  private normalizeRow(row: Record<string, unknown>): Record<string, Sqlite3TableCellValue> {
    return Object.fromEntries(
      Object.entries(row).map(([key, value]) => [key, this.normalizeCellValue(value)]),
    );
  }
  ...
  private createDisplaySql(sql: string, values: readonly unknown[]): string {
    let valueIndex = 0;
    return sql.replaceAll("?", () => this.formatLiteralForDisplay(values[valueIndex++]));
  }
