...
import type { Sqlite3QueryRows, Sqlite3RuntimeDatabase } from "./Sqlite3RuntimeTypes";
...
export class Sqlite3ExportProvider implements ApplicationSqlite3ExportProvider {
   ...
   */
  private async exportDdl(database: Sqlite3RuntimeDatabase, tableName: string): Promise<string> {
    const rows = await this.all(
      database,
      ["SELECT sql", "FROM sqlite_master", "WHERE type IN ('table', 'view')", "AND name = ?"].join(
        " ",
      ),
      [tableName],
    );
   ...
   */
  private async exportDml(database: Sqlite3RuntimeDatabase, tableName: string): Promise<string> {
    const rows = await this.all(database, `SELECT * FROM ${this.quoteIdentifier(tableName)}`, []);
