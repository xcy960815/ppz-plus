...
export class Mysql2MetadataProvider implements MySqlMetadataProvider {
   ...
   */
  private normalizeDatabaseRows(rows: MySqlQueryRows): readonly MySqlSchemaMetadata[] {
    ...
    return rows
      .map((row) => {
        ...
        const databaseName = (row as Record<string, unknown>).Database;
        return typeof databaseName === "string" ? { name: databaseName } : undefined;
      })
  ...
  private normalizeTableRows(
    ...
    return rows
      .map((row) => {
        ...
        const tableName = typedRow.tableName ?? typedRow.TABLE_NAME ?? typedRow.table_name;
        return typeof tableName === "string" ? { schemaName, name: tableName } : undefined;
      })
