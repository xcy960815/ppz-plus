...
export class Mysql2SqlExecutor implements MySqlSqlExecutor {
  ...
  public async executeSql(
    ...
    try {
      ...
      const primaryResultSet = resultSets[0] ?? this.createEmptyStatementResultSet();
  ...
  private resolveSqlTerminalDriverOptions(
    ...
    const driverOptions = this.mySqlConnectionAdapter.resolveDriverOptions(connection);
  ...
  private appendMultipleStatementsOption(connectionUrl: string): string {
    ...
    return `${connectionUrl}${connectionUrl.includes("?") ? "&" : "?"}multipleStatements=true`;
  }
  ...
  private normalizeResultSets(
    ...
    if (this.isMultipleStatementResult(rows, fields)) {
      return (rows as readonly (MySqlQueryResultRow[] | MySqlStatementResult)[]).map(
        (rowSet, index) =>
    ...
    return [
      this.normalizeResultSet(rows as MySqlQueryResultRow[], fields as readonly MySqlField[]),
    ];
  }
   ...
   */
  private isMultipleStatementResult(rows: MySqlQueryRows, fields: MySqlQueryResultFields): boolean {
    ...
    if (Array.isArray(fields)) {
      return fields.some((fieldSet) => Array.isArray(fieldSet) || fieldSet === undefined);
    }

    return (
      fields === undefined && rows.some((row) => !Array.isArray(row) && this.isStatementResult(row))
    );
  }
   ...
   */
  private isStatementResult(value: unknown): value is MySqlStatementResult {
    return (
      value !== null &&
      typeof value === "object" &&
  ...
  private normalizeResultSet(
    ...
    return {
      ...
      metadata: this.normalizeMetadataFromStatementResult(rows as MySqlStatementResult),
    };
  }
   ...
   */
  private normalizeRow(row: Record<string, unknown>): Record<string, SqlExecutionCellValue> {
   ...
   */
  private extractAffectedRows(result: MySqlStatementResult): number | null {
  ...
  private inferQuerySql(sql: string): boolean {
    const firstKeyword = sql
      ...
      .match(/^[a-z]+/i)?.[0]
      .toUpperCase();
    return ["SELECT", "SHOW", "DESCRIBE", "DESC", "EXPLAIN", "WITH", "TABLE", "VALUES"].includes(
      firstKeyword ?? "",
    );
  }
}
