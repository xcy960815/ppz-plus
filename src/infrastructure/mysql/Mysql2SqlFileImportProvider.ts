 ...
 */
export class Mysql2SqlFileImportProvider implements MySqlSqlFileImportProvider {
  /**
  ...
  public async importSql(
    ...
    try {
      ...
      runtimeConnection = await mysql.createConnection(this.resolveImportDriverOptions(connection));
  ...
  private normalizeMySqlDumpText(sql: string): string {
    ...
    for (const rawLine of sql.replaceAll("\r\n", "\n").split("\n")) {
      ...
      normalizedLines.push(
        delimiter === ";" ? rawLine : this.replaceCustomDelimiterAtLineEnd(rawLine, delimiter),
      );
    }
   ...
   */
  private replaceCustomDelimiterAtLineEnd(line: string, delimiter: string): string {
