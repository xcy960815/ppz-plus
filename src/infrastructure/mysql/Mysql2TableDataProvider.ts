...
export class Mysql2TableDataProvider implements MySqlTableDataProvider {
  ...
  public async listColumns(
    ...
    try {
      return this.listColumnsWithConnection(runtimeConnection, schemaName, tableName);
    } finally {
  ...
  public async listRowPage(
    ...
    try {
      ...
      const rowCountQuery = this.createRowCountQuery(schemaName, tableName, columns, options);
  ...
  public async insertRow(
    ...
    try {
      const insertQuery = this.createInsertRowQuery(schemaName, tableName, values);
      const [result] = await runtimeConnection.query(insertQuery.sql, insertQuery.values);
  ...
  public async updateRow(
    ...
    try {
      const updateQuery = this.createUpdateRowQuery(schemaName, tableName, identityValues, values);
      const [result] = await runtimeConnection.query(updateQuery.sql, updateQuery.values);
  ...
  public async deleteRow(
    ...
    try {
      const deleteQuery = this.createDeleteRowQuery(schemaName, tableName, identityValues);
      const [result] = await runtimeConnection.query(deleteQuery.sql, deleteQuery.values);
   ...
   */
  private normalizeColumnRows(rows: MySqlQueryRows): readonly MySqlTableColumnMetadata[] {
    ...
    return rows
      .map((row) => {
        ...
        return {
          name,
          dataType,
          dateTimePrecision: this.normalizeDateTimePrecision(dateTimePrecision),
          nullable: isNullable === "YES",
          isPrimaryKey: columnKey === "PRI",
          extra: typeof extra === "string" ? extra : "",
        };
      })
      .filter((column): column is MySqlTableColumnMetadata => column !== undefined);
  }
  ...
  private createInsertRowQuery(
    ...
    const columnSql = entries.map(([columnName]) => this.escapeIdentifier(columnName)).join(", ");
    const placeholderSql = entries.map(() => "?").join(", ");
    const executableValues = entries.map(([, value]) => value);
    const displayValues = entries.map(([, value]) => this.formatSqlLiteral(value)).join(", ");
  ...
  private createFilterClause(
    ...
    if (keyword.length > 0) {
      const searchableColumnsSql = columns
        .map((column) => `CAST(${this.escapeIdentifier(column.name)} AS CHAR)`)
        .join(", ");
    ...
    return {
      ...
      displaySql: `WHERE ${clauses.map((clause) => clause.displaySql).join(" AND ")}`,
  ...
  private createFilterConditionClause(
    ...
    switch (condition.operator) {
      ...
      case "in":
      case "not in":
        return this.createSetFilterConditionClause(columnSql, condition.operator, condition.value);
      case "=":
      case "!=":
      case ">":
      case ">=":
  ...
  private createScalarFilterConditionClause(
    ...
    const normalizedValue: MySqlTableCellValue = Array.isArray(value)
      ? String(value[0] ?? "")
      : String(value ?? "");
    ...
    return {
      ...
      displaySql: `${columnSql} ${sqlOperator} ${this.formatSqlLiteral(normalizedValue)}`,
  ...
  private createSetFilterConditionClause(
    ...
    | undefined {
    const values = this.normalizeFilterValueList(value);
    ...
    const displayValues = values.map((item) => this.formatSqlLiteral(item)).join(", ");
   ...
   */
  private formatDateCellValue(value: Date, column?: MySqlTableColumnMetadata): string {
    ...
    if (dataType === "datetime" || dataType === "timestamp") {
      const precision = Math.min(Math.max(column?.dateTimePrecision ?? 0, 0), 3);
   ...
   * 将过滤条件值归一化为非空字符串列表。
   *
   * @param {MySqlTableFilterCondition['value']} value 原始过滤条件值。
   * @returns {string[]} 可用于 SQL 参数的字符串列表。
   */
  private normalizeFilterValueList(value: MySqlTableFilterCondition["value"]): string[] {
    const rawValues = Array.isArray(value) ? value : String(value ?? "").split(",");

    return rawValues.map((item) => String(item).trim()).filter((item) => item.length > 0);
  }
  ...
  private readTotalRowCount(rows: MySqlQueryRows): number {
    ...
    const value = (firstRow as Record<string, unknown>).totalRowCount;
    if (typeof value === "number") {
      return value;
    }
  ...
  private readInsertId(result: unknown): string | number | null {
    ...
    const value = (result as Record<string, unknown>).insertId;
   ...
   */
  private escapeQualifiedTableName(schemaName: string, tableName: string): string {
