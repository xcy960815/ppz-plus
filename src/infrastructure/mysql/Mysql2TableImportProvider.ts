...
import type { TableImportResult, TableImportTarget } from "../../domain/import/TableImportResult";
...
export class Mysql2TableImportProvider implements MySqlTableImportProvider {
  ...
  public async importRows(
    ...
    try {
      ...
      let insertedRows = 0;
      for (let index = 0; index < rows.length; index += Mysql2TableImportProvider.batchSize) {
        throwIfCancellationRequested(cancellationSignal);
        const chunk = rows.slice(index, index + Mysql2TableImportProvider.batchSize);
  ...
  private reportProgress(
    ...
    const percentage = totalRows === 0 ? 0 : Math.round((completedRows / totalRows) * 100);
    progressReporter({
      completedRows,
      totalRows,
      percentage,
   ...
   */
  private createInsertSql(target: TableImportTarget, rows: readonly MySqlTableImportRow[]): string {
    const columns = Object.keys(rows[0] ?? {});
    if (columns.length === 0) {
      throw new Error("导入需要表头行。");
    }

    const tableSql = this.escapeQualifiedTableName(target.schemaName, target.tableName);
    const columnSql = columns.map((columnName) => this.escapeIdentifier(columnName)).join(", ");
