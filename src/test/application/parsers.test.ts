...
suite('JsonDocumentParser', () => {
	...
	test('嵌套对象抛错', () => {
		assert.throws(
			() => parser.parse('[{"name":"Alice","meta":{"key":"val"}}]'),
			/不支持的嵌套值/
		);
	});
...
suite('ImportColumnMapper', () => {
	...
	suite('createDefaultMappings', () => {
		test('同名字段自动映射', () => {
			const mappings = mapper.createDefaultMappings(
				['id', 'name'],
				['id', 'name', 'created_at']
			);
	...
	suite('normalizeMappings', () => {
		...
		test('不传 mappings 时使用默认映射', () => {
			const normalized = mapper.normalizeMappings(
				['name'],
				['name', 'age']
			);
	...
	suite('mapRows', () => {
		test('按映射转换行数据', () => {
			...
			const mappedRows = mapper.mapRows(
				...
				(row, sourceName) =>
					(row as Record<string, MySqlTableInsertValue>)[sourceName]
			);
