...
suite("Domain — DatabaseCapabilityDeclaration", () => {
  test("DATABASE_CAPABILITY_KEYS 包含所有能力键", () => {
    ...
    for (const key of expectedKeys) {
      assert.ok(
        DATABASE_CAPABILITY_KEYS.includes(key as (typeof DATABASE_CAPABILITY_KEYS)[number]),
      );
    }
  });
...
suite("Domain — CancellationSignal", () => {
  ...
  test("throwIfCancellationRequested 取消信号为 false 时不抛错", () => {
    assert.doesNotThrow(() => throwIfCancellationRequested({ isCancellationRequested: false }));
  });
