...
export class ExportPostgreSqlDatabaseSqlCommand implements ExtensionCommand {
	 ...
	 */
	public static readonly exportBothId =
		'ppz-plus.exportPostgreSqlDatabaseBoth';
	 ...
	 *
	 * @param databaseNode 当前选中的 PostgreSQL database Tree 节点。
	 */
	private async exportDatabase(
		...
		try {
			const document = await this.exportDatabaseDocumentWithProgress(
				databaseNode,
				targetName
			);
