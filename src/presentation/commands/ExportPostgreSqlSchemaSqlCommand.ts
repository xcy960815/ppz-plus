...
import type { PostgreSqlSchemaTreeNode } from "../explorer/DatabaseConnectionsTreeNode";
...
export class ExportPostgreSqlSchemaSqlCommand implements ExtensionCommand {
   ...
   *
   * @param {ExportPostgreSqlSchemaSqlCommandConfig} config 命令标识和导出内容类型配置。
   ...
   *
   * @param {PostgreSqlSchemaTreeNode} schemaNode 当前选中的 PostgreSQL schema Tree 节点。
   */
  private async exportSchema(schemaNode: PostgreSqlSchemaTreeNode): Promise<void> {
    const startedAt = new Date();
    const targetName = `${schemaNode.databaseName}.${schemaNode.schemaName}`;
    try {
      const document = await this.exportSchemaDocumentWithProgress(schemaNode, targetName);
      ...
      const saveResult = await this.saveSchemaDocumentWithProgress(document, filePath, targetName);
