...
export class ExportMySqlSchemaSqlCommand implements ExtensionCommand {
   ...
   *
   * @param {ExportMySqlSchemaSqlCommandConfig} config 命令标识和导出内容类型配置。
  ...
  public register(): vscode.Disposable {
    return vscode.commands.registerCommand(this.id, async (schemaNode?: MySqlSchemaTreeNode) => {
      if (!schemaNode || schemaNode.kind !== "schema") {
        await vscode.window.showInformationMessage("请选择一个 MySQL schema 节点后再导出 SQL。");
        return;
      }
  ...
  private async exportSchema(schemaNode: MySqlSchemaTreeNode): Promise<void> {
    ...
    const targetName = schemaNode.schemaName;
    try {
