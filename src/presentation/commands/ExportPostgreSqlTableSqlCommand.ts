...
import type { PostgreSqlTableTreeNode } from "../explorer/DatabaseConnectionsTreeNode";
...
export class ExportPostgreSqlTableSqlCommand implements ExtensionCommand {
  ...
  public register(): vscode.Disposable {
    return vscode.commands.registerCommand(this.id, async (tableNode?: PostgreSqlTableTreeNode) => {
      if (!tableNode || tableNode.kind !== "postgresqlTable") {
        await vscode.window.showInformationMessage("请选择一个 PostgreSQL 表节点后再导出 SQL。");
        return;
      }

      const capabilityCheck = this.checkSqlExportCapabilityUseCase.execute("postgresql", this.kind);
