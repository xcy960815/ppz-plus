...
export class AddSqlite3ConnectionCommand implements ExtensionCommand {
	...
	private async createConnection(): Promise<void> {
		...
		try {
			...
			if (shouldTest === '测试连接') {
				await withConnectionTestProgress(config, () =>
					this.testConnectionUseCase.execute(config)
				);
				await vscode.window.showInformationMessage(
					`已连接到 SQLite3 数据库“${config.name}”。`
				);
			}
		} catch (error) {
	...
	public static async collectSqlite3Config(
		...
		const selectedFiles = await vscode.window.showOpenDialog({
			...
			canSelectMany: false,
			defaultUri: existingConfig
				? vscode.Uri.file(existingConfig.dbPath)
				: undefined,
			filters: {
