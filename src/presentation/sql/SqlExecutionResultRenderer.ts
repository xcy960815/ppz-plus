...
export class SqlExecutionResultRenderer {
   ...
   */
  private resolveResultSets(result: SqlExecutionResult): readonly SqlExecutionResultSet[] {
   ...
   */
  private renderMultipleResultSets(resultSets: readonly SqlExecutionResultSet[]): string {
    return resultSets
      .map((resultSet, index) => {
  ...
  private renderDataTable(
    ...
    const headers = fields.map((field) => `<th>${this.escapeHtml(field.name)}</th>`).join("");
    const rowMarkup =
      rows.length === 0
   ...
   */
  private renderKeyValueTable(metadata: readonly SqlExecutionResultMetadataEntry[]): string {
