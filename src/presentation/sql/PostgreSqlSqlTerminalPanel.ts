	...
	implements ExtensionActivationParticipant, vscode.WebviewPanelSerializer
{
	...
	private async handleWebviewMessage(
		...
		state.selectedDatabaseName =
			message.databaseName && message.databaseName.length > 0
	...
	private async renderHtml(
		...
		const selectedDatabaseName =
			state.selectedDatabaseName ??
			(selectedConnection
				? this.resolveConfiguredDatabaseName(selectedConnection)
				: undefined);
...
<body>
		...
		<div class="header">
			<div>
				...
				<div class="subtitle">${this.escapeHtml(
					this.describeSelectedConnection(
						connections,
						selectedConnectionId,
						selectedDatabaseName
					)
	...
	private parseSerializedState(
		value: unknown
	): PostgreSqlSqlTerminalSerializedState {
		...
		return {
			selectedConnectionId:
				typeof selectedConnectionId === 'string'
					...
					: undefined,
			selectedDatabaseName:
				typeof selectedDatabaseName === 'string' &&
	 ...
	 */
	private async listPostgreSqlConnections(): Promise<
		readonly PostgreSqlConnectionConfig[]
	> {
		const connections = await this.listStoredConnectionsUseCase.execute();
		return connections.filter(
			(connection): connection is PostgreSqlConnectionConfig =>
	...
	private resolveConfiguredDatabaseName(
		...
		try {
			const databaseName = decodeURIComponent(
				new URL(connection.url).pathname.replace(/^\//, '')
			);
