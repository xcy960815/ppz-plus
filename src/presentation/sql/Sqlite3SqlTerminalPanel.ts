	...
	implements ExtensionActivationParticipant, vscode.WebviewPanelSerializer
{
	...
	private async handleWebviewMessage(
		...
		state.result = await this.executeSqlite3SqlUseCase.execute(
			selectedConnection,
			message.sql
		);
		await this.render(state);
...
<body>
	...
	private async listSqlite3Connections(): Promise<Sqlite3ConnectionConfig[]> {
		...
		return connections.filter(
			(connection): connection is Sqlite3ConnectionConfig =>
	...
	private parseSerializedState(
		value: unknown
	): Sqlite3SqlTerminalSerializedState {
		...
		return {
			selectedConnectionId:
				typeof record.selectedConnectionId === 'string'
					? record.selectedConnectionId
					: undefined,
