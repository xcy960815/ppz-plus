...
export class PrepareMySqlCsvImportMappingUseCase {
  ...
  public async execute(
    ...
    try {
      const csv = this.csvDocumentParser.parse(await this.csvFileReader.readText(filePath.trim()));
      ...
      return {
        ...
        targetFields,
        defaultMappings: this.importColumnMapper.createDefaultMappings(csv.headers, targetFields),
      };
    } catch (error) {
      return {
        success: false,
