...
export class ExportMySqlTablesBatchUseCase {
  ...
  public async execute(
    ...
    for (const [index, table] of tables.entries()) {
      ...
      try {
        const document = await this.exportMySqlTableUseCase.execute(connection, table, input.kind);
   ...
   */
  private calculatePercentage(completedItems: number, totalItems: number): number {
    if (totalItems <= 0) {
      return 100;
    }
