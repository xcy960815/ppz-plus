import type { JsonDocument, JsonDocumentRow, JsonDocumentValue } from "./JsonDocument";
...
export class JsonDocumentParser {
   ...
   */
  private normalizeCellValue(value: unknown, rowNumber: number, key: string): JsonDocumentValue {
    if (
      value === null ||
      typeof value === "string" ||
      typeof value === "number" ||
    ...
    throw new Error(`JSON 第 ${rowNumber} 行字段“${key}”包含不支持的嵌套值。`);
  }
}
