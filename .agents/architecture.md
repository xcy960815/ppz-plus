...
- 连接配置存储拆分为“可上传的非敏感配置”和“仅本机保存的敏感信息”两部分。
- 密码等敏感字段统一放在 VS Code `SecretStorage`，不进入远端同步载荷。
- 本地连接列表继续由统一的连接仓储接口提供，仓储负责把非敏感配置与本机密码重新拼装成运行时连接对象。
- 远端同步不依赖 VS Code 自动设置同步；由扩展显式提供“上传连接配置”“拉取连接配置”命令控制。
- 后续若增加远端同步实现，建议在 `Application` 层定义同步用例与远端适配接口，在 `Infrastructure` 层落具体后端。
- 表现层在连接被实际使用、或手动拉取完成后发现缺少本机密码时，再提示用户补录，不在领域层写兼容兜底。
